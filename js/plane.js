...
window.onload = function(){
	...
	var queue = [
		...
		{
			name: "planeTexture",
			// url: "./assets/textures/matcap/untitled.6.jpg",type:assetsLoader.IMG
			url: "./assets/textures/matcap/thuglee-03.jpg",type:assetsLoader.IMG
		},
		{
			name: "plane",
			url: "./assets/models/binaries/plane.js",type:assetsLoader.MOD
		},
		{
			name: "particles",
			url:"./assets/models/particles/plane/plane65k.txt", type:assetsLoader.TXT,
			onLoad:function (txt) {
				var obj = {};
				var res = txt.split( '|' );
...
function createMaterials(){
	...
	materials.plane = new THREE.ShaderMaterial({
		uniforms:{
			tMatCap : {type:"t", value:assetsLoader.planeTexture },
			time:{type:"f", value:0 },
			alpha:{type:"f", value:.85 }
		},
...
function createMeshes() {
	...
	assetsLoader.plane.computeVertexNormals();
    plane = new THREE.Mesh(assetsLoader.plane, materials.plane );
...
function createParticles(){
	...
	if( assetsLoader.particles === undefined ){
		...
		var model = plane;
		var count = Math.pow( 2, 16 );//->65536

		assetsLoader.particles = Scatter.distribute( model, count );
		Scatter.toString( assetsLoader.particles, 3, "plane"+~~(count/1000)+"k" );
